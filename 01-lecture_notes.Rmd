...
```{lemma}
There exists Householder refelctions $H_1, \dots, H_r$ such that $H_r \dots H_1 A\Pi = R$. 
```
...
2) The QR decomposition of $[A b]$ gives $\begin{bmatrix} R & Q'b \\ 0 & S \end{bmatrix}$ (ex. \@ref(exr:q407) and \@ref(exr:q408))
...
Suppose $A \in \R^{n\times m}$ and $n\ge m$. Then there exists $U \in \R^{n \times n}$ and $U \in \R^{m\times m}$ that are both orthogonal, and a diagonal matrix $\Sigma \in \R^{n \times m}$ with diagonal elements $\sigma_1 \ge \sigma_2 \ge \dots \ge \sigma_m \ge 0$ such that
