...
$$\norm{(I-F)^{-1}}_p \leq \sum{k=1}^\infty \norm{F}_p^k = \frac{1}{1- \norm{F}_p}$$. 
...
a) Gram-Schmidt requires $2nm^2$ computations. 
...
```{exercise, q901}
What do we need from A to ensure that the different iterative method shcemes are well-defined?
```
