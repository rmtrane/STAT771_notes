...
According to strategy 2, we choose $\alpha$ such that it minimizes $\norm{x_{k+1} - x^*}_2^2$. So, to minimize this, we differentiate with respect to $\alpha$, set to $0$, and solve for $\alpha$:

$$\begin{aligned}
  \frac{d}{d\alpha}\norm{x_{k+1} - x^*}_2^2 &= \frac{d}{d\alpha}\norm{x_k - \alpha A^\prime () - x^*} \\
  &= -2(A^\prime r_k)^\prime \left(x_k - x^* - \alpha A^\prime r_k \right) \\
  &= -2 r_k^\prime A(x_k-x^*) + 2\alpha (A^\prime r_k)^\prime A^\prime r_k \\
  &= -2 r_k^\prime A(x_k-x^*) + 2\alpha \norm{A^\prime r_k}_2^2 = 0 \\
  \Rightarrow \alpha &= \frac{r_k^\prime A(x_k - x^*)}{\norm{A^\prime r_k}}
\end{aligned}$$
...
```{exercise, q1301}
Show that the procedure in \@ref(conjugatedgradients) is the same as the one implemented in code provided by Patel.
```
...
```{exercise, q1403}
Solve $x = G(x)$ by hand. What is $G(x)$ doing? 
```
